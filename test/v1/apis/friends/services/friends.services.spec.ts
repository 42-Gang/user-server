...
describe('FriendsService', () => {
  ...
  describe('request', () => {
    it('정상적인 친구 요청을 처리해야 함', async () => {
      ...
      (mockFriendRepository.findByUserIdAndFriendId as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  describe('accept', () => {
    it('정상적으로 친구 요청을 수락해야 함', async () => {
      ...
      (mockFriendRepository.update as ReturnType<typeof vi.fn>)
        .mockResolvedValue(undefined);
      (mockFriendRepository.create as ReturnType<typeof vi.fn>)
        .mockResolvedValue(undefined);
    ...
    it('친구 요청이 존재하지 않으면 NotFoundException을 던져야 함', async () => {
      ...
      await expect(() => friendsService.accept(2, 1)).rejects.toThrowError(
    ...
    it('요청 상태가 PENDING이 아니면 ConflictException을 던져야 함', async () => {
      ...
      await expect(() => friendsService.accept(2, 1)).rejects.toThrowError(
        new ConflictException('Only pending requests can be accepted'),
      );
    });
  });
  ...
  describe('reject', () => {
    ...
    it('정상적인 친구 요청 거절을 처리해야 함', async () => {
      ...
      (mockFriendRepository.updateStatus as ReturnType<typeof vi.fn>)
        .mockResolvedValue(undefined);
    ...
    it('userId가 undefined이면 NotFoundException을 던져야 함', async () => {
      await expect(() =>
        friendsService.reject(undefined, 1),
  ...
  describe('block', () => {
    ...
    it('친구 상태가 ACCEPTED가 아니면 예외를 던져야 함', async () => {
      ...
      (mockFriendRepository.findByUserIdAndFriendId as ReturnType<typeof vi.fn>)
        .mockResolvedValue(pendingFriend);
    
      await expect(friendsService.block(1, 2)).rejects.toThrowError('Only accepted friends can be blocked');
    });
  });
  ...
  describe('unblock', () => {
    describe('unblock', () => {
      ...
      it('친구 상태가 BLOCKED가 아니면 예외를 던져야 함', async () => {
        ...
        await expect(friendsService.unblock(1, 2)).rejects.toThrowError('Only blocked friends can be unblocked');
      });
    });
  });
});
